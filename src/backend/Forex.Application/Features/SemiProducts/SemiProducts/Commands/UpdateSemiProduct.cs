...
) : IRequestHandler<UpdateSemiProductCommand, bool>
{
    public async Task<bool> Handle(UpdateSemiProductCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.Photo is not null)
            semiProduct.ImagePath = await UploadNewPhoto(semiProduct, request, cancellationToken);
    ...
    {
        if (!string.IsNullOrEmpty(semiProduct.ImagePath))
            await fileStorage.DeleteAsync(semiProduct.ImagePath, ct);
